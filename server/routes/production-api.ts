...
try {
  ...
  dao.ensureQuotesTablesCreated().then(() => {
    console.log('✓ Quotes tables ready');
  }).catch((err: Error) => {
    console.error('✗ Failed to create quotes tables:', err.message);
  });
} catch (error) {
...
router.get('/api/production/orders/:id', async (req, res) => {
  try {
    ...
    const sourceQuote = await dao.getQuoteByOrderId(orderId).catch(() => null);
    ...
    const response = {
      ...
      source_quote: sourceQuote ? {
        ...
        issue_date: sourceQuote.issue_date,
...
// POST /api/quotes/:id/convert - 見積書から受注を作成
...
// GET /api/quotes/:id/items - 見積明細一覧
router.get('/api/quotes/:id/items', async (req, res) => {
  try {
...
// POST /api/quotes/:id/items - 見積明細追加
...
// PUT /api/quotes/:id/items/:itemId - 見積明細更新（ownership確認付き）
router.put('/api/quotes/:id/items/:itemId', async (req, res) => {
  try {
    const quoteId = parseInt(req.params.id, 10);
...
// DELETE /api/quotes/:id/items/:itemId - 見積明細削除（ownership確認付き）
